...
void OrdenadorDeFichas::actualizar(const Communication::GameState &gameState) {
    ...
    for (auto it = gameState.enemies.begin();
         it != gameState.enemies.end(); ++it)
        actualizarEnemigo(*it);
    for (auto it = gameState.towers.begin(); it != gameState.towers.end(); ++it)
        actualizarTorre(*it);
    for (auto it = gameState.positionalPowers.begin();
         it != gameState.positionalPowers.end(); ++it)
        actualizarEfectos(*it);
    for (auto it = gameState.targetPowers.begin();
         it != gameState.targetPowers.end(); ++it)
        actualizarEfectos(*it);
    for (auto it = poderes.begin(); it != poderes.end(); ++it)
...
void OrdenadorDeFichas::actualizarTorre(Communication::Tower actualzacion) {
    ...
    if (actualzacion.is_attacking) {
        ...
        if (it != enemigos.end()) {
            ...
            agregarEfectos(actualzacion.id, actualzacion.current_target_id,
                           idEfectos, sprites);
            idEfectos++;
        }
    }
...
void OrdenadorDeFichas::agregarEfectos(int inicio, int objetivo,
    ...
    FichaEfectos nuevaFicha = FichaEfectos(getTorre(inicio), 1, sprites,
                                           getEnemigo(objetivo));
...
void
OrdenadorDeFichas::actualizarEfectos(Communication::TargetPower actualzacion) {
    idEfectos++;
    switch (actualzacion.type) {
...
void OrdenadorDeFichas::cargarMapa(std::string &mapa) {
    ...
    for (int i = 1; i <= y; i++) {
        for (int j = 1; j <= x; j++) {
            id++;
            agregarTerreno(
                    FichaTerreno(largo * i - 44, largo * j - 44, id, tematica,
                                 sprites));
        }
    }
    ...
    for (auto it = pisosFirmes.begin(); it != pisosFirmes.end(); ++it) {
        id = ObetenerTerrenoEnEstaPosicion((it->x) * 88 - 44,
                                           (it->y) * 88 - 44);
        if (id != 0) {
    ...
    for (auto &pathEnemigo : pathEnemigos) {
        ...
        if (pathEnemigo.entry.x == 0)
            agregarPortal(FichaPortal(-44, (pathEnemigo.entry.y - 1) * largo, 1,
                                      FichaPortalAzul2, sprites));
        if (pathEnemigo.entry.x == x + 1)
            agregarPortal(FichaPortal((pathEnemigo.entry.x - 1) * largo - 44,
                                      (pathEnemigo.entry.y - 1) * largo, 1,
                                      FichaPortalAzul2, sprites));
        if (pathEnemigo.entry.y == 0)
            agregarPortal(FichaPortal((pathEnemigo.entry.x - 1) * largo, -44, 1,
                                      FichaPortalAzul1, sprites));
        if (pathEnemigo.entry.y == y + 1)
            agregarPortal(FichaPortal((pathEnemigo.entry.x - 1) * largo,
                                      (pathEnemigo.entry.y - 1) * largo - 44, 1,
                                      FichaPortalAzul1, sprites));

        if (pathEnemigo.exit.x == 0)
            agregarPortal(FichaPortal(-44, (pathEnemigo.exit.y - 1) * largo, 2,
                                      FichaPortalRojo2, sprites));
        if (pathEnemigo.exit.x == x + 1)
            agregarPortal(FichaPortal((pathEnemigo.exit.x - 1) * largo - 44,
                                      (pathEnemigo.exit.y - 1) * largo, 1,
                                      FichaPortalRojo2, sprites));
        if (pathEnemigo.exit.y == 0)
            agregarPortal(FichaPortal((pathEnemigo.exit.x - 1) * largo, -44, 1,
                                      FichaPortalRojo1, sprites));
        if (pathEnemigo.exit.y == y + 1)
            agregarPortal(FichaPortal((pathEnemigo.exit.x - 1) * largo,
                                      (pathEnemigo.exit.y - 1) * largo - 44, 1,
                                      FichaPortalRojo1, sprites));
                ...
                if (it2->x == (it2 + 1)->x) {
                    ...
                    for (int yy = start; yy <= end; ++yy) {
                        id = ObetenerTerrenoEnEstaPosicion((it2->x) * 88 - 44,
                                                           yy * 88 - 44);
                        if (id != 0)
                            getTerreno(id).cambiarTipo(FichaPisoEnemigos,
                                                       sprites);
                    }
                } else {
