...
void Sprite::dibujar(const Cairo::RefPtr<Cairo::Context> &cr) {
    ...
    cr->rectangle(x, y, x + image->get_width(),
                  y + image->get_height()); //esto es importante.
    cr->fill();
    cr->restore();
    cr->save();
}
...
void Sprite::dibujarIsometrico(const Cairo::RefPtr<Cairo::Context> &cr,
    ...
    if (x2 < (0 - LimiteDeDibujoX) ||
        x2 > (datosActuales.width + LimiteDeDibujoX) ||
        y2 < (0 - LimiteDeDibujoY) ||
        y2 > (datosActuales.height + LimiteDeDibujoY))
        return;

    int correctorX = image->get_width() / 2, correctorY =
            image->get_height() / 2;
    ...
    cr->rectangle(x2 - correctorX, y2 - correctorY,
                  x2 + image->get_width() + correctorX,
                  y2 + image->get_height() + correctorY);
